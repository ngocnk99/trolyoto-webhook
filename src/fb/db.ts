...
// BRAND_ALIASES + resolveBrandAliasFromText: tách sang `./brandAliases.ts`
// (module thuần, zero-dependency) để test được bằng ts-node không cần DB —
// xem `src/fb/__tests__/brandAliases.test.ts`. Re-export ở đây để giữ nguyên
// import path cho các module khác (vd `v3/flow-handler.ts`).
export { resolveBrandAliasFromText } from './brandAliases'
